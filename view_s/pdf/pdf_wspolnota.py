...
from create_pdf_documents.models import Wspolnota, Mieszkaniec, Naliczenie_jeden_mieszkaniec, Naliczenie_cala_wspolnota
...
@login_required
def pdf_wspolnota(request, my_id):
    response = HttpResponse(content_type='application/pdf')
    tytul_data = str(Naliczenie_cala_wspolnota.objects.get(id=my_id).data_dokumentu)
    tytul_wspolnota = Naliczenie_cala_wspolnota.objects.get(id=my_id).wspolnota
    id_wspolnota = Naliczenie_cala_wspolnota.objects.get(id=my_id).wspolnota.id
    pdf_name = f'{str(tytul_wspolnota).replace(" ","")}_{tytul_data}.pdf'
    intab = "ąęółńśźż"
    outtab = "aeolnszz"
    ...
    data = Naliczenie_cala_wspolnota.objects.get(id=my_id).data_dokumentu
    data_poczatek_oplat = Naliczenie_cala_wspolnota.objects.get(id=my_id).data_obowiazywania_oplat
    data_koniec_oplat = Naliczenie_cala_wspolnota.objects.get(id=my_id).oplaty_do
    zmiana = Naliczenie_cala_wspolnota.objects.get(id=my_id).zmianie_ulegaja_oplaty_za
    stawka_eksploatacja = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_zaliczka_eksploatacyjna
    stawka_CO_miernik = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_CO_miernik
    stawka_cw = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_podgrzanie_wody
    stawka_zw = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_zimna_woda
    stawka_scieki = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_odprowadzenie_sciekow
    stawka_nieczystosci = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_wywoz_nieczystosci
    stawka_winda = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_winda
    stawka_miejsce_postojowe_komorka = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_miejsce_postojowe_komorka
    stawka_fundusz_remontowy = Naliczenie_cala_wspolnota.objects.get(id=my_id).stawka_fundusz_remontowy
    informacja = Naliczenie_cala_wspolnota.objects.get(id=my_id).dodatkowa_informacja_gora
    informacja_dol = Naliczenie_cala_wspolnota.objects.get(id=my_id).dodatkowa_informacja_dol
    informacja_dol_2 = Naliczenie_cala_wspolnota.objects.get(id=my_id).dodatkowa_informacja_dol_2
    ...
    if data_koniec_oplat > data_poczatek_oplat:
        if data_poczatek_oplat.day == 1 and data_koniec_oplat.day == ilosc_dni_w_miesiacu:
            ...
            p.drawString(50, 490, f'3.  Gdy naliczenie obejmuje okres krótszy niż 1 miesiąc, to miesiąć początka i końca')
            p.drawString(55, 475, f'opłat powinny być takie same.')
        if rob == 0:
            None
        else:
            ta_wspolonota = Wspolnota.objects.get(id=id_wspolnota)
            for i in Mieszkaniec.objects.filter(wspolnota=ta_wspolonota).order_by('adres_numer_mieszkania'):
                if data_poczatek_oplat >= i.aktywny_od and data_poczatek_oplat <= i.aktywny_do \
                        and data_poczatek_oplat >= i.dane_od and data_poczatek_oplat <= i.dane_do and \
                        data_koniec_oplat <= i.aktywny_do:
                    ilosc_dodanych_do_naliczenia_wlascicieli = ilosc_dodanych_do_naliczenia_wlascicieli + 1
                    imie = i.imie
                    nazwisko = i.nazwisko
                    ulica = i.adres_ulica
                    dom = i.adres_numer_domu
                    mieszkanie = i.adres_numer_mieszkania
                    wspolnota = i.wspolnota
                    ...
                    powierzchnia = i.powierzchnia_mieszkania
                    if stawka_CO_miernik == 0:
                        zaliczka_CO = i.zaliczka_CO
                    else:
                        zaliczka_CO = i.zaliczka_CO_miernik
                    zaliczka_cw = i.podgrzanie_wody_objetosc
                    zaliczka_zw = i.zimna_woda_objetosc
                    zaliczka_scieki = i.odprowadzenie_sciekow_objetosc
                    osoby = i.ilosc_osob
                    winda = i.winda
                    zaliczka_uchyl_wody = i.uchyl_wody
                    zaliczka_miejsce_postojowe_komorka = i.miejsce_postojowe_komorka
                    p.setFont("DejaVuSans", 10, leading=None)
                    ...
                    if procent == 1:
                        ...
                        if stawka_CO_miernik == 0:
                            ...
                            s9 = float(str(f'{stawka_miejsce_postojowe_komorka * zaliczka_miejsce_postojowe_komorka:.2f}'))
                        ...
                        p.setFont("DejaVuSans", 10, leading=None)
                        if stawka_eksploatacja != 0 or powierzchnia != 0:
                            p.drawString(268, 433, f'{stawka_eksploatacja:.2f} zł x {ilosc_dni_oplat}/{ilosc_dni_w_miesiacu}')
                        ...
                        if zaliczka_CO != 0:
                            if stawka_CO_miernik == 0:
                                p.drawString(362, 415, f'{zaliczka_CO:.2f} zł x {ilosc_dni_oplat}/{ilosc_dni_w_miesiacu}')
                                p.drawString(480, 415, f'{zaliczka_CO * procent:.2f} zł')
                            else:
                                p.drawString(378, 415,
                                             f'{zaliczka_CO:.2f} [GJ]')
                                p.drawString(265, 415, f'{stawka_CO_miernik:.2f} zł x {ilosc_dni_oplat}/{ilosc_dni_w_miesiacu}')
                        ...
                        if stawka_winda != 0:
                            ...
                            p.drawString(480, 325, f'------')
                        if stawka_fundusz_remontowy != 0 or powierzchnia != 0:
                            p.drawString(268, 222, f'{stawka_fundusz_remontowy:.2f} zł x {ilosc_dni_oplat}/{ilosc_dni_w_miesiacu}')
                        ...
                        if stawka_CO_miernik == 0:
                            ...
                            s9 = float(
                                str(f'{stawka_miejsce_postojowe_komorka * zaliczka_miejsce_postojowe_komorka * procent:.2f}'))
                            p.drawString(480, 270,
                                     f'{s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9:.2f} zł')
                        else:
                            s1 = float(str(f'{stawka_eksploatacja * powierzchnia * procent:.2f}'))
    ...
    if data_poczatek_oplat < data_koniec_oplat and ilosc_dodanych_do_naliczenia_wlascicieli == 0 and licznik_braku_dodanych_wlascicieli == 0:
        p.setFont("DejaVuSans-Bold", 14, leading=None)
        p.drawString(40, 720, f'BŁĄD:')
        p.drawString(40, 700, f'Dla podanego okresu naliczenia żaden z właścicieli')
        ...
        p.drawString(40, 640, f'i/lub sprawdź dla właścicieli wspólnoty')
        p.drawString(40, 620, f'dla której tworzone jest naliczenie,')
